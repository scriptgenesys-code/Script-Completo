...
(function() {
    ...
    // --- Mapa de Tags, Mapeamento de Categorias, Config ---
    // (O código dos mapas, categorias e config permanece o mesmo...)
    ...
    // --- Funções Utilitárias (makeDraggable, etc) ---
    // (O código das funções utilitárias permanece o mesmo...)
    const findEl = (selectors, parent = document) => { if (!Array.isArray(selectors)) selectors = [selectors]; for (const selector of selectors) { try { if (!parent?.querySelector) continue; const el = parent.querySelector(selector); if (el) return el; } catch (e) { log("Erro sel:", selector, e); } } return null; };
    const copyTextToClipboard = async (text) => { try { await navigator.clipboard.writeText(text); return true; } catch (err) { log('Falha copiar:', err); return false; } };
    ...
    const loadData = async () => {
        ...
        try {
            ...
            if (localProtocols) {
                ...
                if (Array.isArray(parsed) && parsed.length > 0) {
                    ...
                    log("LocalStorage de protocolos estava inválido ou vazio. Removido.");
                }
            }
        } catch (e) {
        ...
        if (!protocolsLoaded) {
            ...
            try {
                // Adiciona ?v=Date.now() para evitar cache
                ...
                if (Array.isArray(data)) {
                    ...
                    // Formato de 'protocolos_brisanet_v1_9.json'
                    newProtocolos = data.protocols;
                    log("JSON em formato Objeto { protocols: [] } (protocolos_brisanet.json) detetado.");
                } else {
                    throw new Error("Formato do JSON do GitHub é inválido.");
                }

                CONFIG.PROTOCOLOS = newProtocolos;
                log(`Protocolos (${newProtocolos.length}) puxados com sucesso do GitHub.`);
                
                // Salvar no LocalStorage para a próxima vez
                try {
                    localStorage.setItem('pr_protocolos', JSON.stringify(newProtocolos));
                    log("Protocolos do GitHub salvos no LocalStorage para uso futuro.");
                } catch (saveErr) {
                    log("Erro ao salvar protocolos no LocalStorage:", saveErr);
                    UI.createNotification("Protocolos carregados, mas falha ao salvar localmente.", 'warn', 4000);
                }

            } catch (fetchErr) {
                log("ERRO CRÍTICO ao puxar protocolos do GitHub:", fetchErr);
                CONFIG.PROTOCOLOS = initialProtocolos; 
                try { UI.createNotification("Falha ao carregar protocolos do GitHub.", 'error', 5000); } catch(e){}
            }
        }

        // 3. Carregar o resto das configurações
        try {
            ...
            log("Erro ao carregar outras configs:", e);
        ...
        CONFIG.PROTOCOLOS.forEach(reply => {
            reply.requiredData = (Array.isArray(reply.requiredData) ? reply.requiredData : []).map(tag => typeof tag === 'string' ? tag.replace(/[{}]/g, '').replace(/ /g, '_').toUpperCase().replace(/[ÁÀÂÃÄ]/g,'A').replace(/[ÉÈÊË]/g,'E').replace(/[ÍÌÎÏ]/g,'I').replace(/[ÓÒÔÕÖ]/g,'O').replace(/[ÚÙÛÜ]/g,'U').replace(/Ç/g,'C').replace(/[^A-Z0-9_]/g, '') : '');
    ...
    const logUsedProtocolo = (title, text) => {
        const MAX = 5; const entry = { title, text }; CONFIG.RECENT_PROTOCOLOS = CONFIG.RECENT_PROTOCOLOS.filter(r => r.title !== title || r.text !== text); CONFIG.RECENT_PROTOCOLOS.unshift(entry); if (CONFIG.RECENT_PROTOCOLOS.length > MAX) CONFIG.RECENT_PROTOCOLOS.length = MAX;
    };

    // --- Extração de Dados ---
    const documentExtractor = {
        getParticipantName: () => { const sels = ['h3.participant-name', 'div.interaction-name-wrapper div.participant-name > span', 'p.participant-name', 'span[data-qa-id="participant-name"]', 'h2.participant-name', '.top-section-block .title-wrapper .title-text', '.conversation-header .participant-info .name']; const el = findEl(sels); if (!el) return 'cliente'; const raw = el.textContent.trim(); const match = raw.match(/^([a-zA-Z\sáéíóúâêîôûãõçÁÉÍÓÚÂÊÎÔÛÃÕÇ.-]+)(?:\s*[|•-]?\s*(?:\(?\d{2}\)?\s*\d{4,5}[-.\s]*\d{4,5}|\d{8,14}|\w+@\w+\.\w+))?$/); return (match?.[1]?.trim() || raw) || 'cliente'; }, getInteractionId: () => { const sels = ['span[data-qa-id="interaction-id"]', '[data-qa-id="detail-value-interactionId"]', '.interaction-id-value', '#interactionIdDisplay']; const el = findEl(sels); return el?.textContent.trim().replace(/#/g, '') || 'protocolo'; }
     };
    ...
    const keyboardEventHandler = (e) => {
        const target = e.target.tagName.toUpperCase(); const isPopupInput = e.target.closest('.pr-script-base-popup') && (target === 'INPUT' || target === 'TEXTAREA' || target === 'SELECT'); if (e.key !== 'Escape' && isPopupInput) return; if (e.ctrlKey && e.shiftKey && e.key.toUpperCase() === 'U') { e.preventDefault(); log("Atalho Ctrl+Shift+U"); UI.createProtocoloPopup(); } if (e.key === 'Escape') { const p1 = document.querySelector('.pr-script-popup'); const p2 = document.querySelector('.pr-script-settings-panel'); if (p1) { log("ESC fecha popup"); p1.remove(); e.preventDefault(); } else if (p2) { log("ESC fecha config"); p2.remove(); e.preventDefault(); } }
    };

    // --- Interface do Usuário (UI) ---
    // (O resto do objeto UI permanece o mesmo do ficheiro original)
    // ... (UI.createNotification, UI.createProtocoloPopup, UI.createSettingsPanel, etc.) ...
    const UI = {
        createNotification(message, type = 'info', duration = 3000) {
            let cont = document.getElementById('pr-script-notification-container'); if (!cont) { cont = document.createElement('div'); cont.id = 'pr-script-notification-container'; document.body.appendChild(cont); } const n = document.createElement("div"); n.textContent = message; n.className = `pr-script-notification pr-script-notification-${type}`; cont.prepend(n); const vis = cont.querySelectorAll('.pr-script-notification'); if (vis.length > 5) vis[vis.length - 1].remove(); setTimeout(() => { n.style.opacity='0'; n.style.transform='translateX(100%)'; setTimeout(() => n.remove(), 300); }, duration);
        },
                ...
                <div class="pr-popup-body">
                    ...
                    <div id="pr-editor-sidebar" style="display: none;">
                        ...
                        <div class="pr-editor-grid">
                            <div class="pr-editor-field"><label for="pr-editor-titular">Nome Titular:</label><input type="text" id="pr-editor-titular"></div>
                            <div class="pr-editor-field">
                                <label for="pr-editor-gender">Gênero (Titular/Contato):</label>
                        ...
                        <div class="pr-editor-field">
                            <label for="pr-editor-relationship">Relação do Contato:</label>
                        ...
                        <div class="pr-editor-field" id="pr-editor-contact-name-group" style="display: none;"> <label for="pr-editor-contact-name">Nome Contato (se não for titular):</label> <input type="text" id="pr-editor-contact-name"> </div>
                        <div class="pr-editor-field"><label for="pr-editor-phone">Telefone Contato:</label><input type="tel" id="pr-editor-phone" placeholder="(XX) XXXXX-XXXX"></div>
                        <div class="pr-editor-grid">
                            <div class="pr-editor-field">
                                <label for="pr-editor-date">Data (Agendamento/Visita):</label>
                                <input type="date" id="pr-editor-date">
                            </div>
                            <div class="pr-editor-field">
                                <label for="pr-editor-timeslot">Período Visita:</label>
                                <select id="pr-editor-timeslot"> <option value="nao_definido" selected>Não Definido</option> <option value="manha">Manhã</option> <option value="tarde">Tarde</option> <option value="noite">Noite</option> </select>
                            </div>
                        </div>
                        <div class="pr-editor-field">
                            <label for="pr-editor-notes">Notas Rápidas (opcional):</label>
                            <textarea id="pr-editor-notes" rows="2" placeholder="Adicionar observação ao final..."></textarea>
                        </div>
                        <button id="pr-editor-preview-btn" class="pr-script-button">Gerar Pré-visualização</button>
                        <textarea id="pr-editor-preview" rows="6" placeholder="Pré-visualização do protocolo modificado..."></textarea>
                        <button id="pr-editor-copy-preview-btn" class="pr-script-button">Copiar Pré-visualização</button>
                    </div>
                </div>`;
            document.body.appendChild(popup); popup.style.width = '850px';
            makeDraggable(popup, popup.querySelector('.panel-header'), 'protocoloPopup'); popup.querySelector('.pr-script-popup-close-btn').onclick = () => popup.remove(); popup.querySelector('.settings-btn').onclick = () => UI.createSettingsPanel();
             ...
             previewBtn.addEventListener('click', () => {
                 try {
                     ...
                     if (relationshipValue === 'proprio') {
                         ...
                         if (gender !== 'neutro') {
                             modifiedText = modifiedText.replace(/\b(o|a)\s+mesm[oa]\b/gi, `${pronounEnding} mesm${pronounEnding}`);
                             modifiedText = modifiedText.replace(/\bclient[ea]\b/gi, `client${gender === 'feminino' ? 'a' : 'e'}`);
                         }
                     } else if (contactName) {
                         ...
                         contactPerson = relationshipText;
                     }
                     const contactRegex = /^(?:Própri[ao]\s+titular|Cliente)\b/i;
                     if (contactPerson && contactRegex.test(modifiedText.trimStart())) {
                         modifiedText = modifiedText.replace(contactRegex, contactPerson);
                     } else if (contactPerson && !modifiedText.trimStart().toLowerCase().startsWith(contactPerson.toLowerCase())) {
                         log("Adicionando contactPerson no início pois regex falhou.");
                         modifiedText = contactPerson + ". " + modifiedText.trimStart();
                     }
                     ...
                     if (phoneNumber) {
                         ...
                         if (phoneMatch) {
                             modifiedText = modifiedText.replace(phoneAddRegex, phoneText);
                         } else {
                              const nameEndRegex = new RegExp(`^${escapeRegExp(contactPerson)}(\\s*[.,;]?)`, 'i');
                              if (contactPerson && nameEndRegex.test(modifiedText.trim())) {
                                  modifiedText = modifiedText.replace(nameEndRegex, (match, p1) => `${contactPerson}, contato ${phoneNumber}${p1 || '.'}`);
                              } else {
                                 modifiedText = `Contato ${phoneNumber}. ${modifiedText}`;
                             }
                         }
                     } else if (phoneMatch) {
                          modifiedText = modifiedText.replace(phoneAddRegex, `entrou em contato${phoneMatch[2] || ''}`);
                     }
                     let visitInfo = '';
                     if (visitDateRaw) {
                         try {
                             const [year, month, day] = visitDateRaw.split('-');
                             if (day && month && year && year.length === 4) {
                                 visitInfo += ` no dia ${day}/${month}/${year}`;
                             }
                         } catch (e) { log("Erro formatar data", e)}
                     }
                     if (timeslotValue !== 'nao_definido') {
                         visitInfo += (visitInfo ? ', ' : ' no ') + `período da ${timeslotSelect.options[timeslotSelect.selectedIndex].text}`;
                     }
                     if (visitInfo) {
                         ...
                         let replacedDate = false;
                          modifiedText = modifiedText.replace(visitPlaceholderRegex, (match, p1, p2) => {
                              replacedDate = true;
                              let finalPunctuation = (p2 && p2.trim().match(/[.,!?;]$/)) ? p2.trim() : '.';
                              return `${p1}${visitInfo}${finalPunctuation}`;
                          });
                          if (!replacedDate) {
                              modifiedText = modifiedText.trimEnd().replace(/[.,!?;]$/, '') + visitInfo + '.';
                          }
                     }
                     if (quickNotes) {
                     ...
                     log("Erro ao gerar preview:", err);
                     UI.createNotification("Erro ao gerar pré-visualização.", "error");
                     previewTextArea.value = originalTextArea.value;
                 }
             });
            copyPreviewBtn.addEventListener('click', async () => {
                 const textToCopy = previewTextArea.value; if (!textToCopy) { UI.createNotification("Nada na pré-visualização para copiar.", 'warn'); return; } if (await copyTextToClipboard(textToCopy)) { UI.createNotification("Pré-visualização Copiada!", 'success'); } else { UI.createNotification("Erro ao copiar pré-visualização.", 'error'); }
            });
        ...
        createSettingsPanel() {
            ...
            panel.querySelector('#pr-import-json-btn').onclick = () => {
                try {
                        ...
                        reader.onload = (event) => {
                            try {
                                ...
                                if (Array.isArray(parsedData)) {
                                    newProtocolos = parsedData;
                                    log("Importando JSON formato Lista (Array).");
                                } else if (parsedData && Array.isArray(parsedData.protocols)) {
                                     newProtocolos = parsedData.protocols;
                                    log("Importando JSON formato Objeto (extraindo 'protocols').");
                                } else {
                                     throw new Error("JSON inválido. Esperado uma lista [ ] ou um objeto { \"protocols\": [...] }.");
                                }

                                if (!Array.isArray(newProtocolos)) {
                                    throw new Error("Falha ao extrair a lista de protocolos do JSON.");
                                }

                                const isValid = newProtocolos.every(p => p.title && p.text);
                                if (!isValid) {
                                    throw new Error("JSON inválido (protocolos sem 'title' ou 'text').");
                                }
                                if (confirm(`Importar ${newProtocolos.length} protocolos? Isto substituirá os atuais.`)) {
                                    CONFIG.PROTOCOLOS = newProtocolos.map(r => ({ ...r, isFavorite: r.isFavorite || false, requiredData: r.requiredData || [] }));
                                    Object.keys(requiredTagsMap).forEach(key => delete requiredTagsMap[key]);
                                    CONFIG.PROTOCOLOS.forEach(reply => {
                                        reply.requiredData = (Array.isArray(reply.requiredData) ? reply.requiredData : []).map(tag => typeof tag === 'string' ? tag.replace(/[{}]/g, '').replace(/ /g, '_').toUpperCase().replace(/[ÁÀÂÃÄ]/g,'A').replace(/[ÉÈÊË]/g,'E').replace(/[ÍÌÎÏ]/g,'I').replace(/[ÓÒÔÕÖ]/g,'O').replace(/[ÚÙÛÜ]/g,'U').replace(/Ç/g,'C').replace(/[^A-Z0-9_]/g, '') : '');
                                        requiredTagsMap[reply.title] = reply.requiredData;
                                    ...
                                    renderSettings();
                                    UI.createNotification("Protocolos importados! Clique em 'Salvar'.", "success", 4000);
                                }
                            } catch (err) {
                                log("Erro ao processar o JSON:", err);
                                UI.createNotification(`Erro ao importar: ${err.message}`, "error", 5000);
                            } finally {
                    ...
                    log("Erro ao iniciar importação:", e);
                    UI.createNotification("Erro ao iniciar a importação.", "error");
                }
            };
    ...
    // --- Injeção de CSS ---
    // (O código CSS original permanece o mesmo)
    const injectCss = () => {
        const css = `
            ...
            /* --- Editor Lateral --- */
            #pr-editor-sidebar h5 { margin: 0 0 10px 0; color: var(--accent-primary); text-align: center; border-bottom: 1px solid var(--border-color); padding-bottom: 8px; font-size: 1em; font-weight: 600;}
            .pr-editor-field { margin-bottom: 10px; }
            .pr-editor-field label { display: block; margin-bottom: 4px; font-weight: 500; color: var(--accent-secondary); font-size: 0.88em; }
            .pr-editor-field input[type="text"],
            .pr-editor-field input[type="tel"],
            .pr-editor-field input[type="date"],
            #pr-editor-preview,
            .pr-editor-field select,
            #pr-editor-notes { width: 100%; padding: 8px; border: 1px solid var(--border-color); border-radius: 4px; box-sizing: border-box; font-size: 0.95em; background-color: var(--bg-medium); color: var(--text-primary); font-family: 'Segoe UI', 'Roboto', Arial, sans-serif; }
            .pr-editor-field input[type="date"] { color-scheme: dark; }
            .pr-editor-field input[type="date"]::-webkit-calendar-picker-indicator { filter: invert(0.8); cursor: pointer; }
            .pr-editor-field input[type="text"]:focus,
            .pr-editor-field input[type="tel"]:focus,
            .pr-editor-field input[type="date"]:focus,
            #pr-editor-preview:focus,
            .pr-editor-field select:focus,
             #pr-editor-notes:focus { border-color: var(--accent-secondary); box-shadow: 0 0 8px rgba(0, 170, 255, 0.5); outline: none; }
            .pr-editor-field select { cursor: pointer; appearance: none; -webkit-appearance: none; padding-right: 30px; background-image: url('data:image/svg+xml;charset=US-ASCII,%3Csvg%20xmlns%3D%22http%3A%2F%2Fwww.w3.org%2F2000%2Fsvg%22%20width%3D%22292.4%22%20height%3D%22292.4%22%3E%3Cpath%20fill%3D%22%2300f0ff%22%20d%3D%22M287%2069.4a17.6%2017.6%200%200%200-13-5.4H18.4c-5%200-9.3%201.8-12.9%205.4A17.6%2017.6%200%200%200%200%2082.2c0%205%201.8%209.3%205.4%2012.9l128%20127.9c3.6%203.6%207.8%205.4%2012.8%205.4s9.2-1.8%2012.8-5.4L287%2095c3.5-3.5%205.4-7.8%205.4-12.8%200-5-1.9-9.2-5.5-12.8z%22%2F%3E%3C%2Fsvg%3E'); background-repeat: no-repeat; background-position: right .9em top 50%; background-size: .7em auto; }
            #pr-editor-preview-btn { width: 100%; margin: 10px 0 5px 0; }
            #pr-editor-copy-preview-btn { width: 100%; margin: 0 0 8px 0; background-color: #6c757d; }
            #pr-editor-copy-preview-btn:hover { background-color: #8a9197;}
            #pr-editor-preview { background-color: var(--bg-medium); font-family: 'Segoe UI', 'Roboto', Arial, sans-serif; font-size: 0.9em; color: var(--text-secondary); min-height: 80px; margin-top: 0px; }
            /* Grid para campos lado a lado no editor */
             .pr-editor-grid { display: grid; grid-template-columns: 1fr 1fr; gap: 10px; }
    ...
    const initialize = async () => { // <-- TORNAR ASYNC
        try {
            ...
            await loadData(); // <-- ADICIONAR AWAIT
        } catch (error) {
            log("ERRO load:", error);
    ...
    const tryInitialize = async () => { // <-- TORNAR ASYNC
        if (findEl('body')) {
            log("Body OK. Init.");
            await initialize(); // <-- ADICIONA AWAIT
        } else {
            log("Wait body...");
