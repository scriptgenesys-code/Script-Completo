// ==UserScript==
// @name         PureCloud - Menu Unificado (V6.0 - Master)
...
(function() {
    ...
    const css = `
        /* Esconde os botões originais de cada módulo para não poluir a tela */
        #qr-trigger-button, 
        #pr-trigger-button, 
        #bau-trigger-button, 
        #gemini-float-btn, 
        ...
        #uni-btn-bar { background: linear-gradient(135deg, #ff6b6b, #ee5253); } /* Vermelho - Ferramentas */
        #uni-btn-pr  { background: linear-gradient(135deg, #d4fc79, #96e6a1); color: #333; } /* Verde Claro - Protocolos */
        #uni-btn-qr  { background: linear-gradient(135deg, #00f260, #0575e6); } /* Verde/Azul - Respostas */
        #uni-btn-ia  { background: linear-gradient(135deg, #2563EB, #7C3AED); } /* Azulão - IA */
        #uni-btn-mon { background: linear-gradient(135deg, #F59E0B, #D97706); } /* Laranja - Monitor */
        #uni-btn-central { background: linear-gradient(135deg, #06B6D4, #3B82F6); } /* Ciano - Central */
    ...
    // 2. HTML DA ESTRUTURA
    if (document.getElementById('uni-menu-container')) {
        document.getElementById('uni-menu-container').remove();
    }
    ...
    // 3. LÓGICA DE CLIQUE E ARRASTO (Destravada)
    ...
    function onMouseMove(e) {
        ...
        // Só move se passar de 5px de deslocamento
    ...
    // Abre o menu apenas se não foi um arrasto
    ...
    } catch (e) { console.log('Posição Menu padrão.'); }

    // 4. TRIGGERS (Ativa os módulos invisíveis)
    function triggerModule(id) {
        const btn = document.getElementById(id);
        if (btn) {
            btn.click();
        } else {
            console.warn("Módulo não carregado: " + id);
        }
    }
    ...
    document.getElementById('uni-btn-id').onclick = () => {
        if (typeof window.executarExtracaoDocumento === 'function') {
            ...
            alert("Módulo Extrator não está pronto.");
        }
    };
