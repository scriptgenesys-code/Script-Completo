// ==UserScript==
// @name         PureCloud - Menu Unificado (V8.3 - Radical Clean)
// @namespace    http://tampermonkey.net/
// @version      8.3
// @description  Menu FAB com limpeza radical de duplicatas.
...
(function() {
    ...
    // --- 1. FAXINA GERAL (KILL SWITCH) ---
    const MENU_ID = 'uni-menu-container';
    
    // Remove qualquer menu anterior (ID ou Classe)
    document.querySelectorAll('#uni-menu-container').forEach(el => el.remove());
    document.querySelectorAll('.uni-fab-main').forEach(el => el.remove()); // Remove botões perdidos
    
    // Remove estilos anteriores
    const oldStyle = document.getElementById('uni-menu-style');
    if (oldStyle) oldStyle.remove();

    // --- 2. CSS ---
    const css = `
        /* Garante que botões antigos originais sumam para evitar "fantasmas" */
        ...
        #uni-menu-container { 
            ...
            width: 60px; height: auto; bottom: 20px; right: 20px; font-family: sans-serif;
        }
        ...
        .uni-fab-main { 
            ...
            transition: transform 0.3s; user-select: none; pointer-events: auto;
        }
        ...
        .uni-fab-btn { 
            ...
            transition: transform 0.2s; position: relative; pointer-events: auto;
        }
        .uni-fab-btn:hover { transform: scale(1.15); z-index: 9999; }
        ...
        #uni-btn-car { background: linear-gradient(135deg, #10B981, #3B82F6); }
    ...
    // --- 4. ARRASTO E EVENTOS ---
    const container = document.getElementById(MENU_ID);
    ...
    document.getElementById('uni-btn-car').onclick = () => { if (typeof window.toggleCAR === 'function') window.toggleCAR(); else alert("CAR não carregado."); };
    document.getElementById('uni-btn-id').onclick = () => { if (typeof window.executarExtracaoDocumento === 'function') window.executarExtracaoDocumento(); else alert("Extrator não pronto."); };
    document.getElementById('uni-btn-copy').onclick = () => { if (typeof window.executarCopiaEspelho === 'function') { const btn = document.getElementById('uni-btn-copy'); btn.style.transform = 'scale(0.9)'; setTimeout(()=>btn.style.transform = 'scale(1)', 150); window.executarCopiaEspelho(); } else alert("Espelho não carregado."); };
