...
// 1. CARREGA A CONFIGURAÇÃO DO COMPATIBILITY.JS
...
document.addEventListener("DOMContentLoaded", () => {
    // PROTEÇÃO: Se a URL não existir, mostra erro amigável
    if (!API_URL) {
...
function exportarRelatorio() {
    ...
    // Formata Atendimentos
    atendimentosFiltrados.forEach(item => {
        listaExportacao.push({
            Data: item.data || obterDataLocal(),
            Hora: item.hora,
            Tipo: "ATENDIMENTO",
            Agente: item.agente,
            Cliente: item.cliente,
            TMA: item.tma || "00:00",
            Status: item.isRecorrencia ? "Recorrência" : "Único",
            Link: item.link || "N/A"
        });
    });

    // Formata Encerrados
    encerradosFiltrados.forEach(item => {
        listaExportacao.push({
            Data: obterDataLocal(),
            Hora: item.hora,
            Tipo: "ENCERRAMENTO MANUAL",
            Agente: item.atendente || item.agente || "Desconhecido",
    ...
    link.setAttribute("download", `${nomeArquivo}_${obterDataLocal()}.json`);
...
function processarRecorrencia(lista) {
    let unicos = new Set(); let listaProcessada = []; let countUnicos = 0;
    lista.sort((a, b) => (a.data+a.hora).localeCompare(b.data+b.hora));
    lista.forEach(item => {
        const d = item.data || obterDataLocal();
        const c = (item.cliente || "sem_nome").trim().toLowerCase();
        const t = item.telefone || "";
        const chave = `${d}_${c}_${t}`;
        if (unicos.has(chave)) item.isRecorrencia = true;
        else { item.isRecorrencia = false; unicos.add(chave); countUnicos++; }
        listaProcessada.push(item);
    });
    return { lista: listaProcessada, totalUnicos: countUnicos };
}
...
async function carregarDados() {
    ...
    try {
        const container = document.getElementById("lista-agentes-container");
        container.innerHTML = "<div class='loading' style='text-align:center; padding:20px'>Carregando...</div>";
        ...
        if (data.permissoes && (data.permissoes.role === "ADMIN" || data.permissoes.setor === "ADMIN")) {
            document.getElementById("btn-gestao").style.display = "block";
            if(data.usuariosGestao) preencherSelectGestao(data.usuariosGestao);
        } else { document.getElementById("btn-gestao").style.display = "none"; }

        todosAgentes = data.agentes || [];
        todosAtendimentos = data.detalhesAtendimentos || [];
        todosEncerrados = data.detalhesEncerrados || [];

        preencherDropdown(todosAgentes);
        renderizarGrafico(todosAgentes);
        renderizarListaGlobal(todosAgentes);
        filtrarVisao(agenteSelecionadoGlobal);
        setTimeout(() => btn.style.transform = "rotate(0deg)", 500);
    } catch (e) {
        console.error(e);
        document.getElementById("lista-agentes-container").innerHTML = "<div style='text-align:center; color:var(--danger)'>Erro Conexão</div>";
    }
}
...
function filtrarVisao(agente) {
    ...
    const viewIndiv = document.getElementById("view-individual");
    document.getElementById("titulo-agente-selecionado").innerText = "👤 " + agente;
    let la, lat, lenc, countU = 0;
    if (agente === "todos") {
        viewGeral.style.display = "block"; viewIndiv.style.display = "none";
        la = todosAgentes; lat = todosAtendimentos; lenc = todosEncerrados; countU = lat.length; 
    } else {
        viewGeral.style.display = "none"; viewIndiv.style.display = "block";
        la = todosAgentes.filter(a => compararNomes(a.nome, agente));
        let rawLat = todosAtendimentos.filter(i => compararNomes(i.agente, agente));
        const proc = processarRecorrencia(rawLat);
        lat = proc.lista; countU = proc.totalUnicos;
        lenc = todosEncerrados.filter(i => compararNomes(i.atendente, agente) || compararNomes(i.agente, agente));
        renderizarListaDetalhe("lista-individual-atendimentos", lat);
        renderizarListaDetalhe("lista-individual-encerrados", lenc);
    }
    atualizarKPIs(la, countU, lenc.length, agente !== "todos");
}

function atualizarKPIs(a, ca, ce, ind) {
    const el = document.getElementById("total-logados");
    if (ind) { if(a.length) { el.innerText=a[0].status==="Online"?"ON":"OFF"; el.style.color=a[0].status==="Online"?"var(--success)":"var(--danger)"; } else el.innerText="-"; } 
    else { el.innerText = a.filter(u => u.status === "Online").length; el.style.color = "var(--primary)"; }
    document.getElementById("total-conversas").innerText = ca;
    document.getElementById("total-encerrados").innerText = ce;
    let m=0; if(ind && a.length) m=tmaToSeconds(a[0].tma); 
    else { let s=0,q=0; a.forEach(x=>{s+=(x.tmaSoma||0); q+=(x.conversas||0)}); m=q>0?Math.floor(s/q):0; }
    document.getElementById("media-tma").innerText = formatarTempo(m);
}

// --- LISTAS ---
function renderizarListaGlobal(lista) {
    const c = document.getElementById("lista-agentes-container"); c.innerHTML = "";
    if(!lista.length) { c.innerHTML="<div style='text-align:center;padding:10px;color:#666'>Vazio.</div>"; return; }
    lista.sort((a,b)=>(parseInt(b.conversas)||0)-(parseInt(a.conversas)||0));
    lista.forEach(ag => {
        if(!ag.nome) return;
        const d = document.createElement("div"); d.className = "agente-row";
        const cor = ag.status==="Online"?"var(--success)":"var(--danger)";
        d.innerHTML = `<div class="agente-info"><span class="agente-nome">${ag.nome}</span><span class="agente-detalhe"><span style="color:${cor}">● ${ag.status||'Off'}</span> | TMA: ${ag.tma||'00:00'}</span></div><div class="agente-valor">${ag.conversas||0}</div>`;
        d.onclick=()=>{document.getElementById("filtro-agente").value=ag.nome; agenteSelecionadoGlobal=ag.nome; filtrarVisao(ag.nome);};
        c.appendChild(d);
    });
}

function renderizarListaDetalhe(id, lista) {
    const c = document.getElementById(id); c.innerHTML = "";
    const isAtend = id.includes("atendimentos");
    const cid = isAtend ? "count-atendimentos" : "count-encerrados";
    const totalVisual = isAtend ? lista.filter(x => !x.isRecorrencia).length : lista.length;
    document.getElementById(cid).innerText = totalVisual;

    if(!lista.length) { c.innerHTML="<div style='padding:10px;text-align:center;color:#666;font-size:12px'>Sem registros.</div>"; return; }

    [...lista].reverse().forEach(i => {
        const d = document.createElement("div"); d.className = "detalhe-item";
        const quem = i.atendente || i.agente || "Sistema";
        const link = i.link ? `<a href="${i.link}" target="_blank" class="link-btn">Link 🔗</a>` : "";
        let tag = (isAtend && i.isRecorrencia) ? `<span class="tag-recorrencia">🔁 Recorrência</span>` : "";
        d.innerHTML = `<div class="item-content"><span class="item-title">${i.cliente || "Cliente"} ${tag}</span><span class="item-sub">🕒 ${i.hora} <span style="margin:0 5px">|</span> 👤 ${quem}</span></div>${link}`;
        c.appendChild(d);
    });
}
...
function abrirModulos(){document.getElementById("modal-modulos").style.display="block";chrome.storage.local.get(['MOD_CRONOMETROS','MOD_PAUSAS','MOD_PROTOCOLOS','MOD_RESPOSTAS','MOD_BAR','MOD_MENU','MOD_IA','MOD_ID'],r=>{document.getElementById("mod-cronometros").checked=r.MOD_CRONOMETROS!==false;document.getElementById("mod-pausas").checked=r.MOD_PAUSAS!==false;document.getElementById("mod-protocolos").checked=r.MOD_PROTOCOLOS!==false;document.getElementById("mod-respostas").checked=r.MOD_RESPOSTAS!==false;document.getElementById("mod-bar").checked=r.MOD_BAR!==false;document.getElementById("mod-menu").checked=r.MOD_MENU!==false;document.getElementById("mod-ia").checked=r.MOD_IA!==false;document.getElementById("mod-id").checked=r.MOD_ID!==false;});}
function salvarModulos(){const c={'MOD_CRONOMETROS':document.getElementById("mod-cronometros").checked,'MOD_PAUSAS':document.getElementById("mod-pausas").checked,'MOD_PROTOCOLOS':document.getElementById("mod-protocolos").checked,'MOD_RESPOSTAS':document.getElementById("mod-respostas").checked,'MOD_BAR':document.getElementById("mod-bar").checked,'MOD_MENU':document.getElementById("mod-menu").checked,'MOD_IA':document.getElementById("mod-ia").checked,'MOD_ID':document.getElementById("mod-id").checked};chrome.storage.local.set(c,()=>{alert("Salvo! Recarregue a página.");fecharModais();});}
