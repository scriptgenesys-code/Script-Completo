// ==UserScript==
// @name         Gerenciador de Contagem e Produtividade (v1.0.28_OFFLINE_SYNC)
// @namespace    http://tampermonkey.net/
// @version      1.0.28
// @description  Script focado em Analytics. [v1.0.28: Adicionado Sincronização Offline e Modo Admin]
...
(function() {
    ...
    // --- URL DO CEREBRO (GOOGLE SCRIPT) ---
    const GOOGLE_SCRIPT_URL = "https://script.google.com/macros/s/AKfycbyBkz1XED-bMLDrPX19VMPoHmMB2_WovBb-Pn2HN1MG0P3lQOl6MkVCkcI6_Yo6WiGsEg/exec";

    // --- ATUALIZADO ---
    // v1.0.28: Sistema Offline + Modo Admin Silencioso
    const SCRIPT_VERSION = '1.0.28_OFFLINE_SYNC'; 
    // --- FIM DA ATUALIZAÇÃO ---
    ...
    // ==========================================================
    // SISTEMA DE FILA OFFLINE (DATA SYNCER)
    // ==========================================================
    const DataSyncer = {
        QUEUE_KEY: 'genesys_offline_queue_v1',
        ...
        async send(payload) {
            if (navigator.onLine) {
                try {
                    ...
                    console.warn("[DataSyncer] Falha no envio. Salvando na fila.", e);
                    this._addToQueue(payload);
                    if (typeof UI !== 'undefined') UI.createNotification("Sem conexão. Salvo para envio posterior.", "warn");
                }
            } else {
                console.warn("[DataSyncer] Offline. Salvando na fila.");
                this._addToQueue(payload);
                if (typeof UI !== 'undefined') UI.createNotification("Modo Offline. Dado salvo.", "warn");
            }
        ...
        async processQueue() {
            ...
            console.log(`[DataSyncer] Processando ${queue.length} itens da fila.`);
            let failedQueue = [];
            
            for (let item of queue) {
                try {
                    await this._post(item);
                    await new Promise(r => setTimeout(r, 500)); // Delay para segurança
                } catch (e) {
            ...
            if (failedQueue.length === 0) {
                if (typeof UI !== 'undefined') UI.createNotification("Sincronização offline concluída! ✅", "success");
            }
            this.updateIndicator();
        },
        ...
        updateIndicator() {
            ...
            if (queue.length > 0) {
                if (!indicator) {
                    ...
                    indicator.style.cssText = "position:fixed; bottom:80px; right:20px; background:#ff9800; color:white; padding:5px 10px; border-radius:20px; font-size:12px; z-index:9999; box-shadow:0 2px 5px rgba(0,0,0,0.3); font-weight:bold; cursor:pointer;";
                    indicator.title = "Clique para tentar forçar o envio";
                    indicator.onclick = () => DataSyncer.processQueue();
                    document.body.appendChild(indicator);
                }
                indicator.innerText = `📡 ${queue.length} Pendentes`;
    ...
    const analyticsManager = {
        ...
        calculateStats() {
            ...
            const baloonClicksCount = v4_counters.balao.length;
            ...
            if (completedConvos.length === 0) { 
                return { 
                    ...
                    baloonClicks: baloonClicksCount,
    ...
    async function getNativeInteractionUrl() { 
        ...
        try { originalClipboard = await navigator.clipboard.readText(); } catch (e) { log("WARN: Não foi possível ler o clipboard original (permissão?)."); }
        btn.click(); 
        return new Promise(resolve => { 
            setTimeout(async () => { 
                ...
                try { 
                    ...
                    if (!url || !(url.includes('interactions/') || url.includes('conversations/'))) { url = null; }
                } catch (e) { log("Erro ao ler clipboard para obter link da interação.", e); url = null; } 
                if (originalClipboard && url) { try { await navigator.clipboard.writeText(originalClipboard); } catch(e) { log("WARN: Não foi possível restaurar o clipboard.", e); } }
                resolve(url); 
            }, 200); 
        }); 
    }
    ...
    const actions = {
        copyInteraction: async () => {
            ...
            if (participantName === 'N/A' || !findEl(CONFIG.PURECLOUD_SELECTORS.NATIVE_COPY_BUTTON)) { UI.createNotification(CONFIG.MESSAGES.NOT_FOUND_INTERACTION_DATA + ". O nome do cliente pode estar ausente ou inacessível.", 'error'); return; }
            ...
            if (url && (url.includes('interactions/') || url.includes('conversations/'))) {
                ...
                if (tData) { tData.interactionUrl = url; tData.participantName = participantName; tData.phoneNumber = phoneNumber; UI.createNotification("Link salvo na aba 'Atendimentos'!", 'info', 1500); }
                ...
                if (await copyTextToClipboard(finalCopyText)) { UI.createNotification(CONFIG.MESSAGES.INTERACTION_COPIED, 'success'); } else { UI.createNotification(CONFIG.MESSAGES.COPY_ERROR, 'error'); }
            } else {
                UI.createNotification("Link da interação nativa não encontrado. Nome copiado separadamente.", 'warn');
                if (participantName !== 'N/A' && await copyTextToClipboard(participantName)) { UI.createNotification(`Nome do cliente (${participantName}) copiado como fallback.`, 'info'); }
            }
        },
    ...
    async function handleBalaoClick(e) {
        ...
        if (endChatButton && endChatButton.disabled) { log("Clique ignorado: Botão Balão está bloqueado pelo Lockdown."); return; }
        ...
        try {
            ...
            if (urlFromButton) { interactionUrl = urlFromButton; } else { const id = documentExtractor.getInteractionId(document.body); if (id && id !== 'N/A') { interactionUrl = `${window.location.origin}/directory/conversations/${id}`; } else { interactionUrl = window.location.href.includes('/conversations/') ? window.location.href : 'N/A'; } }
        } catch (error) { log("Erro ao tentar obter link na função handleBalaoClick:", error); }
        v4_counters.balao.push({ timestamp: Date.now(), participantName: participantName, phoneNumber: phoneNumber, interactionUrl: interactionUrl });
        saveV4Counters();
        ...
        updateLockButton: (conv) => {
            ...
            const getScriptIframeDoc = () => {
                ...
                if (scriptIframe && scriptIframe.contentDocument) { return scriptIframe.contentDocument; }
                return null;
            };
            ...
            const addAnalyticsListener = (btn, type) => {
                ...
                btn.addEventListener('click', (e) => {
                    ...
                    if (type === 'TRANSFERIDO') { analyticsManager.logTransferClick(); CONFIG.LAST_ENDED_INTERACTIONS.set(id, { type: 'TRANSFERIDO', timestamp: Date.now() }); } 
                    else if (type === 'AVALIACAO') { analyticsManager.logEvaluatedClick(); CONFIG.LAST_ENDED_INTERACTIONS.set(id, { type: 'AVALIACAO', timestamp: Date.now() }); }
                });
                btn.dataset.gmListenerAdded = true;
            };
            const addBalaoListener = (btn) => { if (btn.dataset.gmBalaoListenerAdded) return; btn.addEventListener('click', handleBalaoClick); btn.dataset.gmBalaoListenerAdded = true; };
            document.querySelectorAll(LOCKDOWN_CONFIG.END_CHAT_BUTTON_SELECTOR.join(',')).forEach(btn => addBalaoListener(btn));
            document.querySelectorAll(CONFIG.PURECLOUD_SELECTORS.TRANSFER_BUTTON_SELECTOR.join(',')).forEach(btn => addAnalyticsListener(btn, 'TRANSFERIDO'));
            document.querySelectorAll(CONFIG.PURECLOUD_SELECTORS.EVALUATION_BUTTON_SELECTOR.join(',')).forEach(btn => addAnalyticsListener(btn, 'AVALIACAO')); 
            if (scriptDoc) { scriptDoc.querySelectorAll(CONFIG.PURECLOUD_SELECTORS.EVALUATION_BUTTON_SELECTOR.join(',')).forEach(btn => addAnalyticsListener(btn, 'AVALIACAO')); }
        },
    ...
    function updateTimerDisplay(timerData) {
        ...
        if (timerData.isPaused) { timerData.div.classList.add('paused-alert'); } else { 
            ...
            if (inactivityOperatorSeconds >= CONFIG.INACTIVITY_OPERATOR_ALERT_SECONDS) { timerData.div.classList.add('inactive-operator-alert'); if (!timerData.operatorAlertPlayed) { soundPlayer.playBeep('inactive_operator'); timerData.operatorAlertPlayed = true; }
            } else if (inactivityClientSeconds >= CONFIG.INACTIVITY_CLIENT_ALERT_SECONDS) { timerData.div.classList.add('inactive-client-alert'); if (!timerData.clientAlertPlayed) { soundPlayer.playBeep('inactive_client'); timerData.clientAlertPlayed = true; }
            } else { timerData.div.classList.add('normal-timer'); timerData.operatorAlertPlayed = false; timerData.clientAlertPlayed = false; } 
        }
    ...
    const timerManager = {
        initTimer(convEl, index, total) {
            let tData = conversationTimers.get(convEl); if (!tData) { let bubblesContainer = convEl.querySelector('.injected-bubbles-container'); let tDiv = convEl.querySelector('.injected-conversation-timer'); let nDiv = convEl.querySelector('.injected-conversation-number'); let checkmarkDiv = convEl.querySelector('.injected-checkmark'); let recurrenceDiv = convEl.querySelector('.injected-recurrence-icon'); let iconDiv = convEl.querySelector('.injected-interaction-icon');
                if (!bubblesContainer) { 
                    bubblesContainer = document.createElement('div'); bubblesContainer.className = 'injected-bubbles-container'; 
                    nDiv = document.createElement('div'); nDiv.className = 'injected-element injected-conversation-number'; bubblesContainer.appendChild(nDiv); 
                    iconDiv = document.createElement('div'); iconDiv.className = 'injected-element injected-interaction-icon'; iconDiv.innerHTML = '❔'; bubblesContainer.appendChild(iconDiv);
                    tDiv = document.createElement('div'); tDiv.className = 'injected-element injected-conversation-timer'; bubblesContainer.appendChild(tDiv); 
                    let contentWrapper = findEl(CONFIG.PURECLOUD_SELECTORS.CONVERSATION_CONTENT_WRAPPER, convEl);
                    if (contentWrapper) { contentWrapper.appendChild(bubblesContainer); } else { log("WARN: .interaction-content não encontrado. Anexando bolhas ao convEl."); convEl.appendChild(bubblesContainer); }
                    setTimeout(() => { if (contentWrapper && !contentWrapper.contains(bubblesContainer)) { contentWrapper.appendChild(bubblesContainer); } else if (!convEl.contains(bubblesContainer)) { convEl.appendChild(bubblesContainer); } }, 100); 
                } else { 
                    ...
                    if (!iconDiv) { iconDiv = document.createElement('div'); iconDiv.className = 'injected-element injected-interaction-icon'; iconDiv.innerHTML = '❔'; if (nDiv && nDiv.nextSibling) { bubblesContainer.insertBefore(iconDiv, nDiv.nextSibling); } else { bubblesContainer.appendChild(iconDiv); } }
                ...
                const isRecurrence = phoneNumber !== 'N/A' && analyticsManager.getData().conversations.some(conv => conv.phoneNumber === phoneNumber && conv.duration >= CONFIG.CHECKMARK_THRESHOLD_MS);
                if (isRecurrence) { if (!recurrenceDiv) { recurrenceDiv = document.createElement('div'); recurrenceDiv.className = 'injected-element injected-recurrence-icon'; recurrenceDiv.innerHTML = '🔁'; recurrenceDiv.title = 'Cliente recorrente hoje'; bubblesContainer.appendChild(recurrenceDiv); } recurrenceDiv.style.display = 'flex'; } else if (recurrenceDiv) { recurrenceDiv.style.display = 'none'; }
                tData = { div: tDiv, numDiv: nDiv, iconDiv: iconDiv, checkmarkDiv: checkmarkDiv, recurrenceDiv: recurrenceDiv, bubblesContainer: bubblesContainer, element: convEl, creationTime: Date.now(), startTime: Date.now(), pausedAt: Date.now(), isPaused: true, completedVisual: false, tmaLogged: false, activeDuration: 0, activeSessionStart: null, lastOperatorActivityTimestamp: Date.now(), lastCustomerReplyTimestamp: Date.now(), longConvoNotified: false, operatorAlertPlayed: false, clientAlertPlayed: false, participantName: participantName, phoneNumber: phoneNumber, interactionUrl: 'N/A', interactionId: interactionId, isRecurrence: isRecurrence, interactionType: 'unknown' };
        ...
        cleanupTimers(activeConversationElements) {
            const timersToDelete = []; const AGENT_END_WINDOW_MS = 5000;
            conversationTimers.forEach((timerData, convEl) => {
                if (!document.body.contains(convEl)) {
                    log(`Limpando timer para ${timerData.participantName} removido do DOM.`); clearInterval(timerData.intervalId);
                    if (!timerData.tmaLogged) {
                        if (!timerData.isPaused && timerData.activeSessionStart) { timerData.activeDuration += (Date.now() - timerData.activeSessionStart); timerData.activeSessionStart = null; }
                        const duration = Date.now() - timerData.creationTime; const currentTime = Date.now();
                        const clickData = CONFIG.LAST_ENDED_INTERACTIONS.get(timerData.interactionId); let classification = 'HIBERNADO';
                        if (clickData && (currentTime - clickData.timestamp) < AGENT_END_WINDOW_MS) { if (clickData.type === 'BALAO') { classification = 'BALAO'; } else if (clickData.type === 'TRANSFERIDO') { classification = 'TRANSFERIDO'; } else if (clickData.type === 'AVALIACAO') { classification = 'AVALIACAO'; } CONFIG.LAST_ENDED_INTERACTIONS.delete(timerData.interactionId); }
                        analyticsManager.logConversation(duration, timerData.activeDuration, timerData.participantName, timerData.phoneNumber, timerData.interactionUrl, classification, timerData.interactionType);
                        timerData.tmaLogged = true; if (CONFIG.LAST_ENDED_INTERACTIONS.has(timerData.interactionId)) { CONFIG.LAST_ENDED_INTERACTIONS.delete(timerData.interactionId); }
                    }
                    timersToDelete.push(convEl); timerData.div?.remove(); timerData.numDiv?.remove(); timerData.iconDiv?.remove(); timerData.checkmarkDiv?.remove(); timerData.recurrenceDiv?.remove(); timerData.bubblesContainer?.remove();
                }
            });
    ...
    function mainLoop() {
        ...
        sortedConversationsArray.forEach((conv, index) => { 
            const nativeTimeElementsInConv = conv.querySelectorAll(CONFIG.PURECLOUD_SELECTORS.NATIVE_TIMER_SELECTORS.join(', ')); nativeTimeElementsInConv.forEach(el => { if (el.classList.contains('injected-element') || el.classList.contains('injected-bubbles-container')) { return; } if (el.textContent.match(/\d+\s*(min|hr|seg|now|agora)/i) || el.matches('.duration, .message-timestamp')) { el.style.display = 'none'; } });
            const tData = timerManager.initTimer(conv, index, sortedConversationsArray.length); 
            const isCall = conv.querySelector(CONFIG.PURECLOUD_SELECTORS.ICON_SELECTOR_CALL); const isChat = conv.querySelector(CONFIG.PURECLOUD_SELECTORS.ICON_SELECTOR_CHAT);
            if (tData.iconDiv) { if (isCall) { tData.iconDiv.innerHTML = '📞'; tData.iconDiv.title = 'Ligação'; tData.interactionType = 'call'; } else if (isChat) { tData.iconDiv.innerHTML = '💬'; tData.iconDiv.title = 'Chat'; tData.interactionType = 'chat'; } else { tData.iconDiv.innerHTML = '❔'; tData.iconDiv.title = 'Interação'; tData.interactionType = 'unknown'; } }
            const isSelected = conv === selectedConvEl; timerManager.updateState(tData, isSelected); conversationLockManager.initLockState(conv); if (isSelected) { observeCurrentChatMessages(tData); conversationLockManager.updateLockButton(conv); } 
        });
        if (!selectedConvEl) { const allEndButtons = document.querySelectorAll(LOCKDOWN_CONFIG.END_CHAT_BUTTON_SELECTOR.join(',')); allEndButtons.forEach(btn => { btn.disabled = true; btn.classList.add('lockdown-disabled'); btn.title = 'Selecione uma conversa'; }); const allEvalButtons = document.querySelectorAll(CONFIG.PURECLOUD_SELECTORS.EVALUATION_BUTTON_SELECTOR.join(',')); allEvalButtons.forEach(btn => { btn.disabled = true; btn.classList.add('lockdown-disabled'); btn.title = 'Selecione uma conversa'; }); const lockButton = document.getElementById('purecloud-script-btn-toggleLock'); if (lockButton) { const ICON_SHIELD_LOCKED = '<path d="M12 22s8-4 8-10V5l-8-3-8 3v7c0 6 8 10 8 10z"></path>'; lockButton.innerHTML = `<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5" style="width: 18px; height: 18px;">${ICON_SHIELD_LOCKED}</svg>`; lockButton.title = 'Controle de Encerramento de Conversa'; } }
        if (CONFIG.MINI_DASHBOARD_VISIBLE) { UI.createMiniDashboard(); }
    }

    const getHotkeyCombo = (e) => { const pressedKeys = []; if (e.ctrlKey) pressedKeys.push('Control'); if (e.shiftKey) pressedKeys.push('Shift'); if (e.altKey) pressedKeys.push('Alt'); const key = e.key === ' ' ? 'Space' : (e.key.length === 1 ? e.key.toUpperCase() : e.key); if (!['Control', 'Shift', 'Alt', 'Meta'].includes(key)) { pressedKeys.push(key); } const primaryKey = pressedKeys.pop(); pressedKeys.sort(); if (primaryKey) pressedKeys.push(primaryKey); return pressedKeys.join('+'); };
    const keyboardEventHandler = (e) => { if (e.target.tagName === 'INPUT' || e.target.tagName === 'TEXTAREA' || e.target.isContentEditable) { return; } const currentCombo = getHotkeyCombo(e); if (['Control', 'Shift', 'Alt', 'Meta'].includes(e.key)) { return; } for (const actionName in CONFIG.HOTKEYS) { if (CONFIG.HOTKEYS[actionName] === currentCombo) { e.preventDefault(); if (actions[actionName]) { actions[actionName](); UI.createNotification(featureDescriptions[actionName]?.title || actionName, 'info', 1000); return; } } } };

    function createToolbarUI(toolbar) {
        if (toolbar.dataset.scriptVersion === SCRIPT_VERSION) { return; } toolbar.querySelectorAll('.purecloud-script-custom-btn, #purecloud-script-toolbar-counter, .purecloud-script-menu-container').forEach(el => el.remove()); toolbar.dataset.scriptVersion = SCRIPT_VERSION; 
        const createMainToolbarBtn = (id, title, svg, onClick) => { const btn = document.createElement("button"); btn.id = id; btn.className = 'purecloud-script-custom-btn'; btn.title = title; btn.innerHTML = `<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5">${svg}</svg>`; btn.onclick = onClick; btn.classList.toggle('pc-script-dark-mode', CONFIG.DARK_MODE); toolbar.appendChild(btn); return btn; };
        ...
        const ICON_SHIELD_LOCKED = '<path d="M12 22s8-4 8-10V5l-8-3-8 3v7c0 6 8 10 8 10z"></path>'; const lockButton = document.createElement("button"); lockButton.id = 'purecloud-script-btn-toggleLock'; lockButton.className = 'purecloud-script-custom-btn'; lockButton.title = 'Controle de Encerramento de Conversa'; lockButton.innerHTML = `<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5" style="width: 18px; height: 18px;">${ICON_SHIELD_LOCKED}</svg>`; lockButton.onclick = actions.toggleLock; lockButton.classList.toggle('pc-script-dark-mode', CONFIG.DARK_MODE); 
        const endChatButton = findEl(LOCKDOWN_CONFIG.END_CHAT_BUTTON_SELECTOR, toolbar); if (endChatButton) { endChatButton.parentNode.insertBefore(lockButton, endChatButton); } else { toolbar.prepend(lockButton); }
    }

    const UI = {
        createNotification(message, type = 'info', duration = 3000) {
            let c = document.getElementById('purecloud-script-notification-container');
            if (!c) { c = document.createElement('div'); c.id = 'purecloud-script-notification-container'; c.classList.toggle('pc-script-dark-mode', CONFIG.DARK_MODE); document.body.appendChild(c); }
        ...
        renderAnalyticsContent(container) { const stats = analyticsManager.calculateStats(); container.innerHTML = `<div class="stats-grid"><div class="stat-item"><span>Conversas (Únicas)</span><strong id="analytics-total-count">${stats.count}</strong></div><div class="stat-item"><span>TMA (Geral)</span><strong id="analytics-tma">${stats.tma}</strong></div><div class="stat-item"><span>TME (Ativo)</span><strong id="analytics-tme">${stats.tme}</strong></div><div class="stat-item"><span>Balão (Término)</span><strong id="analytics-balao-clicks">${stats.baloonClicks}</strong></div> <div class="stat-item"><span>Início</span><strong id="analytics-first">${stats.first}</strong></div><div class="stat-item"><span>Última</span><strong id="analytics-last">${stats.last}</strong></div><div class="stat-item"><span>Meta</span><strong id="analytics-target">${CONFIG.CONVERSATION_TARGET}</strong></div><div class="stat-item"><span>Transferidos</span><strong id="analytics-transfer-count">${stats.transferClicks}</strong></div></div><p style="font-size: 0.8em; color: var(--purecloud-script-text-secondary); margin-top: 5px;">* Balão (Término) usa o contador integrado (v4.0).</p><h4>Conversas/Hora (Todas Completas)</h4><div class="analytics-chart-container-main"></div>`; UI.renderBarChart(container.querySelector('.analytics-chart-container-main'), stats.perHour); },
        renderDetailsContent(container) {
            const stats = analyticsManager.calculateStats(); const conversations = stats.allConversations.filter(c => c.duration >= CONFIG.CHECKMARK_THRESHOLD_MS).sort((a, b) => b.timestamp - a.timestamp);
            let detailsList = container.querySelector('#details-list'); if (!detailsList) { detailsList = document.createElement('div'); detailsList.id = 'details-list'; container.appendChild(detailsList); }
            if (conversations.length === 0) { detailsList.innerHTML = '<p>Nenhum atendimento completo registrado hoje.</p>'; return; }
            const conversationsAsc = conversations.reverse(); const totalCount = conversationsAsc.length; const recurrenceCount = stats.recurrenceCount;
            detailsList.innerHTML = `
                <div class="details-header" style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 15px;"><h4 style="margin:0; color: var(--purecloud-script-text-primary);">Total de Atendimentos Completos: ${totalCount}</h4><h4 style="margin:0; color: var(--purecloud-script-warn-color); font-weight: bold;">Recorrências Hoje: ${recurrenceCount}</h4></div>
                <style>.details-table { width: 100%; border-collapse: collapse; margin-top: 10px; font-size: 0.85em; } .details-table th, .details-table td { padding: 8px 10px; text-align: left; border-bottom: 1px solid var(--purecloud-script-border-color); } .details-table th { background-color: var(--purecloud-script-bg-secondary); color: var(--purecloud-script-text-primary); } .details-table tr:hover { background-color: rgba(0, 0, 0, 0.05); } .pc-script-dark-mode .details-table tr:hover { background-color: rgba(255, 255, 255, 0.05); } .recurrence-tag, .first-tag { padding: 2px 6px; border-radius: 3px; font-weight: bold; font-size: 0.8em; display: inline-block; } .recurrence-tag { background-color: var(--purecloud-script-warn-color); color: #222; } .first-tag { background-color: var(--purecloud-script-success-color); color: #fff; } .is-recurrence-row { opacity: 0.8; font-style: italic; } .details-table th.col-num, .details-table td.col-num { text-align: right; width: 40px; font-weight: bold; color: var(--purecloud-script-text-secondary); } .details-table th.col-type, .details-table td.col-type { text-align: center; width: 40px; font-size: 1.1em; }</style>
                <table class="details-table"><thead><tr><th class="col-num">#</th><th class="col-type">Tipo</th><th>Hora Fim</th><th>Cliente</th><th>TME</th><th>Recorrência</th><th>Link</th></tr></thead><tbody>${conversationsAsc.map((conv, index) => { const time = getBrazilTime(new Date(conv.timestamp)).toLocaleTimeString('pt-BR', { hour: '2-digit', minute: '2-digit' }); const tmeSeconds = Math.floor(conv.activeDuration / 1000); const tme = `${formatTime(Math.floor(tmeSeconds / 60))}:${formatTime(tmeSeconds % 60)}`; const recurrenceLabel = conv.isRecurrence ? '<span class="recurrence-tag">Sim</span>' : '<span class="first-tag">Não</span>'; const urlLink = conv.interactionUrl && conv.interactionUrl !== 'N/A' ? `<a href="${conv.interactionUrl}" target="_blank" title="Abrir Interação">🔗</a>` : 'N/A'; let typeIcon = '❔'; if (conv.interactionType === 'call') { typeIcon = '📞'; } else if (conv.interactionType === 'chat') { typeIcon = '💬'; } return `<tr class="${conv.isRecurrence ? 'is-recurrence-row' : ''}"><td class="col-num">${index + 1}</td><td class="col-type" title="${conv.interactionType}">${typeIcon}</td><td>${time}</td><td>${conv.participantName || 'N/A'}</td><td>${tme}</td><td>${recurrenceLabel}</td><td>${urlLink}</td></tr>`; }).join('')}</tbody></table><button id="export-details-btn" class="purecloud-script-button" style="margin-top: 15px;">Exportar Detalhes (JSON)</button>`;
            const exportDetailsBtn = container.querySelector('#export-details-btn'); if (exportDetailsBtn) { exportDetailsBtn.onclick = () => { const dataToExport = analyticsManager.getData().conversations; const filename = `purecloud_script_attendances_${getBrazilTime(new Date()).toISOString().split('T')[0]}.json`; const blob = new Blob([JSON.stringify(dataToExport, null, 2)], { type: 'application/json' }); const url = URL.createObjectURL(blob); const a = document.createElement('a'); a.href = url; a.download = filename; document.body.appendChild(a); a.click(); document.body.removeChild(a); URL.revokeObjectURL(url); UI.createNotification("Dados de atendimentos exportados com sucesso!", "success"); }; }
        },
        renderClosedDetailsContent(container) {
            const balaoRecords = v4_counters.balao.slice().reverse();
            container.innerHTML = `
                <h4 style="margin-top:0;">Histórico de Cliques: Balão (Mensagens de Término)</h4><p style="font-size: 0.85em; color: var(--purecloud-script-text-secondary); margin-bottom: 15px;">Total de Cliques de Balão Hoje: <strong style="color: var(--purecloud-script-error-color);">${balaoRecords.length}</strong></p>
                <div id="closed-interactions-list" class="details-table-wrapper" style="max-height: 400px; overflow-y: auto;">${balaoRecords.length === 0 ? '<p>Nenhum clique de Balão registrado hoje.</p>' : ''}<table class="details-table"><thead><tr><th style="width: 20%;">Hora</th><th style="width: 45%;">Cliente</th><th style="width: 25%;">Telefone</th><th style="width: 10%; text-align: center;">Link</th></tr></thead><tbody>${balaoRecords.map(item => { const time = getBrazilTime(new Date(item.timestamp)).toLocaleTimeString('pt-BR', { hour: '2-digit', minute: '2-digit' }); const urlLink = (item.interactionUrl && item.interactionUrl !== 'N/A') ? `<a href="${item.interactionUrl}" target="_blank" title="Abrir Interação">🔗</a>` : 'N/A'; return `<tr><td>${time}</td><td>${item.participantName || 'N/A'}</td><td>${item.phoneNumber || 'N/A'}</td><td style="text-align: center;">${urlLink}</td></tr>`; }).join('')}</tbody></table></div><button id="export-balao-details-btn" class="purecloud-script-button" style="margin-top: 15px;">Exportar Detalhes de Balão (JSON)</button>`;
            const exportDetailsBtn = container.querySelector('#export-balao-details-btn'); if (exportDetailsBtn) { exportDetailsBtn.onclick = () => { const dataToExport = v4_counters.balao; const filename = `purecloud_script_balao_closures_${getBrazilTime(new Date()).toISOString().split('T')[0]}.json`; const blob = new Blob([JSON.stringify(dataToExport, null, 2)], { type: 'application/json' }); const url = URL.createObjectURL(blob); const a = document.createElement('a'); a.href = url; a.download = filename; document.body.appendChild(a); a.click(); document.body.removeChild(a); URL.revokeObjectURL(url); UI.createNotification("Dados de encerramento Balão exportados com sucesso!", "success"); }; }
        },
        ...
        createSettingsPanel() {
            document.querySelector('.purecloud-script-settings-panel')?.remove(); const panel = document.createElement('div'); panel.className = 'purecloud-script-settings-panel purecloud-script-popup'; panel.classList.toggle('pc-script-dark-mode', CONFIG.DARK_MODE); let tempHotkeysConfig = { ...CONFIG.HOTKEYS }; let tempThemeColors = JSON.parse(JSON.stringify(CONFIG.THEME_COLORS));
            ...
            const switchTab = (tabId) => { 
                panel.querySelectorAll('.purecloud-script-tab-button').forEach(btn => btn.classList.remove('active')); panel.querySelectorAll('.purecloud-script-tab-content').forEach(content => content.classList.remove('active')); 
                const targetTabBtn = panel.querySelector(`.purecloud-script-tab-button[data-tab="${tabId}"]`); const targetTabContent = panel.querySelector(`[data-tab-content="${tabId}"]`); 
                if (targetTabBtn) targetTabBtn.classList.add('active'); if (targetTabContent) targetTabContent.classList.add('active'); 
                if (tabId === 'general') { panel.querySelector('#conversation-target').value = CONFIG.CONVERSATION_TARGET; panel.querySelector('#dark-mode-toggle').checked = CONFIG.DARK_MODE; panel.querySelector('#sound-alerts-toggle').checked = CONFIG.SOUND_ALERTS_ENABLED; panel.querySelector('#inactivity-client-alert-seconds').value = CONFIG.INACTIVITY_CLIENT_ALERT_SECONDS; panel.querySelector('#inactivity-operator-alert-seconds').value = CONFIG.INACTIVITY_OPERATOR_ALERT_SECONDS; panel.querySelector('#long-convo-alert-min').value = CONFIG.LONG_CONVO_ALERT_MIN; } else if (tabId === 'analytics') { UI.renderAnalyticsContent(targetTabContent); } else if (tabId === 'details') { UI.renderDetailsContent(targetTabContent); } else if (tabId === 'closed') { UI.renderClosedDetailsContent(targetTabContent); } else if (tabId === 'hotkeys') { renderHotkeys(targetTabContent); } else if (tabId === 'theme') { UI.renderThemeContent(targetTabContent, tempThemeColors); } 
            ...
            generalTab.innerHTML = `<div class="settings-group"><label for="conversation-target">Meta Conversas Diárias:</label><input type="number" id="conversation-target" value="${CONFIG.CONVERSATION_TARGET}" min="1"></div><div class="settings-group"><label>Modo Escuro:</label><label class="switch"><input type="checkbox" id="dark-mode-toggle" ${CONFIG.DARK_MODE ? 'checked' : ''}><span class="slider round"></span></label></div><div class="settings-group"><label>Alertas Sonoros (Inatividade):</label><label class="switch"><input type="checkbox" id="sound-alerts-toggle" ${CONFIG.SOUND_ALERTS_ENABLED ? 'checked' : ''}><span class="slider round"></span></label></div><div class="settings-group"><label for="inactivity-client-alert-seconds">Alerta Inatividade CLIENTE (seg, min 30):</label><input type="number" id="inactivity-client-alert-seconds" value="${CONFIG.INACTIVITY_CLIENT_ALERT_SECONDS}" min="30"></div><div class="settings-group"><label for="inactivity-operator-alert-seconds">Alerta Inatividade OPERADOR (seg, min 10):</label><input type="number" id="inactivity-operator-alert-seconds" value="${CONFIG.INACTIVITY_OPERATOR_ALERT_SECONDS}" min="10"></div><div class="settings-group"><label for="long-convo-alert-min">Alerta Conversa Longa (min, min 5):</label><input type="number" id="long-convo-alert-min" value="${CONFIG.LONG_CONVO_ALERT_MIN}" min="5"></div><div class="settings-group"><h4>Contadores Diários:</h4><button id="reset-counter-btn" class="purecloud-script-button button-danger">Resetar Contadores/Dados do Dia</button><p style="font-size: 0.85em; margin-top: 5px; color: var(--purecloud-script-text-secondary);">Zera contagem, TMA, TME, lista de atendimentos e histórico de encerramentos.</p></div>`; 
            ...
            panel.querySelector('#save-settings-btn').onclick = () => {
                CONFIG.CONVERSATION_TARGET = validateConfigValue('CONVERSATION_TARGET', panel.querySelector('#conversation-target').value); CONFIG.INACTIVITY_CLIENT_ALERT_SECONDS = validateConfigValue('INACTIVITY_CLIENT_ALERT_SECONDS', panel.querySelector('#inactivity-client-alert-seconds').value); CONFIG.INACTIVITY_OPERATOR_ALERT_SECONDS = validateConfigValue('INACTIVITY_OPERATOR_ALERT_SECONDS', panel.querySelector('#inactivity-operator-alert-seconds').value); CONFIG.LONG_CONVO_ALERT_MIN = validateConfigValue('LONG_CONVO_ALERT_MIN', panel.querySelector('#long-convo-alert-min').value); CONFIG.DARK_MODE = validateConfigValue('DARK_MODE', panel.querySelector('#dark-mode-toggle').checked); CONFIG.SOUND_ALERTS_ENABLED = validateConfigValue('SOUND_ALERTS_ENABLED', panel.querySelector('#sound-alerts-toggle').checked); CONFIG.HOTKEYS = validateConfigValue('HOTKEYS', tempHotkeysConfig); CONFIG.THEME_COLORS = validateConfigValue('THEME_COLORS', tempThemeColors);
                saveData(); UI.createNotification("Configurações salvas!", "success"); panel.remove(); document.getElementById('purecloud-script-injected-style')?.remove(); injectCss(); document.getElementById('purecloud-script-mini-dashboard')?.classList.toggle('pc-script-dark-mode', CONFIG.DARK_MODE); document.getElementById('purecloud-script-notification-container')?.classList.toggle('pc-script-dark-mode', CONFIG.DARK_MODE); const toolbarEl = findEl(CONFIG.PURECLOUD_SELECTORS.TOOLBAR); if (toolbarEl) { toolbarEl.dataset.scriptVersion = ''; createToolbarUI(toolbarEl); }
            };
            switchTab('analytics');
        },
        createMiniDashboard() {
            if (!CONFIG.MINI_DASHBOARD_VISIBLE) { const dashboard = document.getElementById('purecloud-script-mini-dashboard'); if (dashboard && dashboard._updateInterval) { clearInterval(dashboard._updateInterval); } dashboard?.remove(); return; }
            let dashboard = document.getElementById('purecloud-script-mini-dashboard');
            if (!dashboard) {
                dashboard = document.createElement('div'); dashboard.id = 'purecloud-script-mini-dashboard'; dashboard.className = 'purecloud-script-popup'; dashboard.classList.toggle('pc-script-dark-mode', CONFIG.DARK_MODE); document.body.appendChild(dashboard);
                dashboard.innerHTML = `<div class="mini-dashboard-header"><div class="counter-display"><span class="counter-text">--/--</span><div class="progress-bar-container-compact"><div class="progress-bar-compact"></div></div></div><div class="mini-dashboard-controls"><button id="mini-dashboard-adjust-attended-btn" class="compact-btn adjust-btn" title="Ajustar Contagem"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" style="width: 14px; height: 14px;"><path d="M11 4H4a2 2 0 0 0-2 2v14a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2v-7"></path><path d="M18.5 2.5a2.121 2.121 0 0 1 3 3L12 15l-4 1 1-4 9.5-9.5z"></path></svg></button><button class="compact-btn toggle-expand-btn" title="Expandir/Recolher"><svg class="expand-icon" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" style="width: 14px; height: 14px;"><path d="M8 3H5a2 2 0 0 0-2 2v3m18 0V5a2 2 0 0 0-2-2h-3m0 18h3a2 2 0 0 0 2-2v-3M3 16v3a2 2 0 0 0 2 2h3"></path></svg><svg class="collapse-icon" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" style="width: 14px; height: 14px; display: none;"><path d="M8 3v3a2 2 0 0 1-2 2H3m18 0h-3a2 2 0 0 1-2-2V3m0 18v-3a2 2 0 0 1 2-2h3M3 16h3a2 2 0 0 1 2 2v3"></path></svg></button><button class="compact-btn close-btn" id="closeMiniDashboardBtn" title="Fechar">X</button></div></div><div class="mini-dashboard-content"><div class="stats-grid-original mini-dashboard-stats-grid"><div class="stat-item-original"><span>Conversas (Únicas)</span><strong id="dashboard-conv-count">--</strong></div><div class="stat-item-original"><span>TMA</span><strong id="dashboard-tma">--:--</strong></div><div class="stat-item-original"><span>TME</span><strong id="dashboard-tme">--:--</strong></div><div class="stat-item-original" title="Cliques no botão 'Mensagens de Término' (Balão)"><span>Balão (Término)</span><strong id="dashboard-balao-clicks">--</strong></div></div></div>`;
                const dashboardHeader = dashboard.querySelector('.mini-dashboard-header'); makeDraggable(dashboard, dashboardHeader, 'miniDashboard');
                dashboard._updateStatsInternal = () => {
                    if (!dashboard.isConnected) { clearInterval(dashboard._updateInterval); return; } const stats = analyticsManager.calculateStats(); const progress = (stats.count / CONFIG.CONVERSATION_TARGET) * 100; const counterTextEl = dashboard.querySelector('.counter-text'); if(counterTextEl) counterTextEl.textContent = `${stats.count}/${CONFIG.CONVERSATION_TARGET}`; const progressBarEl = dashboard.querySelector('.progress-bar-compact'); if(progressBarEl) progressBarEl.style.width = `${Math.min(100, progress)}%`;
                    if (CONFIG.MINI_DASHBOARD_MAXIMIZED) { const currentTmaSeconds = Math.floor(stats.tma.split(':').reduce((acc, time) => (60 * acc) + +time, 0)); const targetTmaSeconds = CONFIG.LONG_CONVO_ALERT_MIN * 60; let tmaIndicator = currentTmaSeconds > 0 ? (currentTmaSeconds < targetTmaSeconds * 0.9 ? '▲' : (currentTmaSeconds > targetTmaSeconds * 1.1 ? '▼' : '●')) : ''; const convCountElement = dashboard.querySelector('#dashboard-conv-count'); if (convCountElement) convCountElement.textContent = stats.count; const tmaElement = dashboard.querySelector('#dashboard-tma'); if (tmaElement) tmaElement.innerHTML = `${stats.tma} <span style="font-size: 0.8em; color: ${tmaIndicator === '▲' ? '#28a745' : tmaIndicator === '▼' ? '#dc3545' : '#ffc107'};">${tmaIndicator}</span>`; const tmeElement = dashboard.querySelector('#dashboard-tme'); if (tmeElement) tmeElement.textContent = stats.tme; const baloonClicksElement = dashboard.querySelector('#dashboard-balao-clicks'); if (baloonClicksElement) baloonClicksElement.textContent = stats.baloonClicks; }
                };
                const toggleButton = dashboard.querySelector('.toggle-expand-btn'); const expandIcon = dashboard.querySelector('.expand-icon'); const collapseIcon = dashboard.querySelector('.collapse-icon');
                const toggleFloatingDashboard = (forceState = null) => {
                    const shouldExpand = forceState !== null ? forceState : !CONFIG.MINI_DASHBOARD_MAXIMIZED; CONFIG.MINI_DASHBOARD_MAXIMIZED = shouldExpand; dashboard.classList.toggle('expanded', shouldExpand); dashboard.classList.toggle('collapsed', !shouldExpand); expandIcon.style.display = shouldExpand ? 'none' : 'inline-block'; collapseIcon.style.display = shouldExpand ? 'inline-block' : 'none'; toggleButton.title = shouldExpand ? "Recolher" : "Expandir";
                    if (!shouldExpand) { dashboard.style.width = ''; dashboard.style.height = ''; dashboard.style.resize = 'none'; } else { const savedPosition = CONFIG.POPUP_POSITIONS['miniDashboard']; if (savedPosition && savedPosition.top && savedPosition.top !== 'auto') { dashboard.style.top = savedPosition.top; dashboard.style.left = savedPosition.left; dashboard.style.bottom = 'auto'; dashboard.style.right = 'auto'; dashboard.style.transform = 'none'; } else { dashboard.style.top = '50%'; dashboard.style.left = '50%'; dashboard.style.transform = 'translate(-50%, -50%)'; dashboard.style.bottom = 'auto'; dashboard.style.right = 'auto'; } dashboard.style.width = '300px'; dashboard.style.height = 'auto'; dashboard.style.resize = 'none'; } saveData(); dashboard._updateStatsInternal();
                };
                dashboard._toggleFloatingDashboard = toggleFloatingDashboard;
                dashboardHeader.onclick = (e) => { if (e.target.closest('.compact-btn, #mini-dashboard-adjust-attended-btn')) { return; } const wasDragging = dashboard.dataset.isDragging === 'true'; dashboard.dataset.isDragging = 'false'; if (!wasDragging && !CONFIG.MINI_DASHBOARD_MAXIMIZED) { toggleFloatingDashboard(true); } };
                ...
                dashboard.querySelector('#mini-dashboard-adjust-attended-btn').onclick = (e) => { e.stopPropagation(); const currentAttendedCount = analyticsManager.calculateStats().count; const newCount = prompt("Insira o novo número de conversas atendidas:", currentAttendedCount); if (newCount !== null && !isNaN(newCount)) { const parsedNewCount = parseInt(newCount, 10); if (parsedNewCount < 0) { UI.createNotification("O número deve ser positivo.", 'error', 4000); return; } let data = analyticsManager.getData(); const currentUniqueCount = analyticsManager.calculateStats().count; if(parsedNewCount < currentUniqueCount) { UI.createNotification("Use 'Resetar Contadores' nas Configurações para reduzir.", "warn", 7000); return; } if (parsedNewCount > currentUniqueCount) { const diff = parsedNewCount - currentUniqueCount; for (let i = 0; i < diff; i++) { data.conversations.push({ timestamp: Date.now() - (i * 1000), duration: CONFIG.CHECKMARK_THRESHOLD_MS + 5000, activeDuration: CONFIG.CHECKMARK_THRESHOLD_MS + 2000, participantName: `Ajuste Manual ${Date.now()}`, phoneNumber: `000000000${i}`, isRecurrence: false, endedByAgent: 'AVALIACAO', interactionUrl: 'Ajuste Manual' }); } analyticsManager.saveData(data); } dashboard._updateStatsInternal(); } };
                toggleFloatingDashboard(CONFIG.MINI_DASHBOARD_MAXIMIZED); if (dashboard._updateInterval) { clearInterval(dashboard._updateInterval); } dashboard._updateInterval = setInterval(dashboard._updateStatsInternal, 5000); dashboard._updateStatsInternal();
            } else { if (dashboard._updateStatsInternal) { dashboard._updateStatsInternal(); } dashboard.classList.toggle('pc-script-dark-mode', CONFIG.DARK_MODE); }
        },
    };

    function injectCss() {
        document.getElementById('purecloud-script-injected-style')?.remove(); const theme = validateConfigValue('THEME_COLORS', CONFIG.THEME_COLORS);
        const css = `
            :root { --purecloud-script-bg-primary: ${theme.bgPrimary}; --purecloud-script-bg-secondary: ${theme.bgSecondary}; --purecloud-script-text-primary: ${theme.textPrimary}; --purecloud-script-text-secondary: ${theme.textSecondary}; --purecloud-script-border-color: ${theme.borderColor}; --purecloud-script-accent-color: ${theme.accent}; --purecloud-script-accent-text: ${theme.accentText}; --purecloud-script-success-color: ${theme.success}; --purecloud-script-error-color: ${theme.error}; --purecloud-script-warn-color: ${theme.warn}; --purecloud-script-info-color: ${theme.info}; --purecloud-script-bubble-bg-normal: ${theme.timerBg}; --purecloud-script-bubble-bg-selected: ${theme.accent}; --purecloud-script-bubble-text-normal: ${theme.timerNormalText}; --purecloud-script-bubble-text-active: ${theme.timerActiveText}; --purecloud-script-bubble-text-paused: ${theme.timerPausedText}; --purecloud-script-bubble-text-timer-inactive: ${theme.timerInactiveClientText}; --purecloud-script-shadow-color: rgba(0,0,0,0.15); --mini-dashboard-progress-color: ${theme.miniDashboardProgress}; --purecloud-script-tag-highlight-color: #f44336; }
            .pc-script-dark-mode { --purecloud-script-bg-primary: #1e1e1e !important; --purecloud-script-bg-secondary: #252526 !important; --purecloud-script-text-primary: #f0f0f0 !important; --purecloud-script-text-secondary: #aaaaaa !important; --purecloud-script-border-color: #3b3b3b !important; --purecloud-script-accent-color: #555555 !important; --purecloud-script-shadow-color: rgba(0,0,0,0.8) !important; }
            .purecloud-script-button { background-color: var(--purecloud-script-accent-color); color: var(--purecloud-script-accent-text); border: none; padding: 10px 15px; border-radius: 4px; cursor: pointer; font-size: 0.9em; font-weight: bold; transition: filter 0.2s ease, transform 0.1s ease, background-color 0.2s; box-shadow: 0 2px 5px rgba(0, 0, 0, 0.2); } .purecloud-script-button:hover { filter: brightness(1.1); }
            .purecloud-script-notification { position: relative; z-index: 10005; width: 320px; padding: 15px 20px; margin: 10px; border-radius: 5px; box-shadow: 0 4px 15px rgba(0,0,0,0.2); color: #fff; font-size: 0.9em; font-weight: 600; opacity: 0.95; transition: opacity 0.3s, transform 0.3s; animation: fadeInRight 0.3s ease-out; background-color: var(--purecloud-script-bg-secondary); color: var(--purecloud-script-text-primary); } #purecloud-script-notification-container { position: fixed; top: 10px; right: 10px; z-index: 10004; } @keyframes fadeInRight { from { opacity: 0; transform: translateX(100%); } to { opacity: 0.95; transform: translateX(0); } } .purecloud-script-notification-success { background-color: var(--purecloud-script-success-color); color: #fff; } .purecloud-script-notification-error { background-color: var(--purecloud-script-error-color); color: #fff; } .purecloud-script-notification-info { background-color: var(--purecloud-script-info-color); color: var(--purecloud-script-text-primary); } .purecloud-script-notification-warn { background-color: var(--purecloud-script-warn-color); color: #222; }
            ...
            .injected-bubbles-container { position: absolute; bottom: 4px; left: 10px; display: flex; gap: 4px; z-index: 10; pointer-events: none; user-select: none; } .interaction-content-wrapper, .interaction-content { position: relative !important; padding-bottom: 25px !important; }
            .injected-conversation-number { min-width: 19px; padding: 1px 7px !important; } .injected-interaction-icon { padding: 1px 4px !important; font-size: 10px !important; }
            .injected-bubbles-container:has(.injected-conversation-timer.normal-timer) .injected-conversation-number, .injected-bubbles-container:has(.injected-conversation-timer.normal-timer) .injected-interaction-icon, .injected-conversation-timer.normal-timer { color: var(--purecloud-script-bubble-text-active) !important; }
            .injected-bubbles-container:has(.injected-conversation-timer.paused-alert) .injected-conversation-number, .injected-bubbles-container:has(.injected-conversation-timer.paused-alert) .injected-interaction-icon, .injected-conversation-timer.paused-alert { color: var(--purecloud-script-bubble-text-paused) !important; }
            .injected-bubbles-container:has(.injected-conversation-timer.inactive-client-alert) .injected-conversation-number, .injected-bubbles-container:has(.injected-conversation-timer.inactive-client-alert) .injected-interaction-icon, .injected-conversation-timer.inactive-client-alert, .injected-bubbles-container:has(.injected-conversation-timer.inactive-operator-alert) .injected-conversation-number, .injected-bubbles-container:has(.injected-conversation-timer.inactive-operator-alert) .injected-interaction-icon, .injected-conversation-timer.inactive-operator-alert { color: var(--purecloud-script-bubble-text-timer-inactive) !important; }
            ...
            .injected-recurrence-icon { background-color: var(--purecloud-script-warn-color) !important; color: #222 !important; font-size: 10px !important; } div.interaction-group.is-selected .injected-recurrence-icon { background-color: var(--purecloud-script-bubble-bg-selected) !important; color: var(--purecloud-script-warn-color) !important; }
            div.interaction-group.is-selected .injected-element { background-color: var(--purecloud-script-bubble-bg-selected) !important; border-color: rgba(255, 255, 255, 0.15) !important; } div.interaction-group.is-selected .injected-bubbles-container:not(:has(.normal-timer, .paused-alert, .inactive-client-alert, .inactive-operator-alert)) .injected-element { color: var(--purecloud-script-accent-text) !important; }
            .theme-section { margin-bottom: 20px; } .theme-section h4 { margin-bottom: 10px; padding-bottom: 5px; border-bottom: 1px solid var(--purecloud-script-border-color); color: var(--purecloud-script-text-primary); } .settings-group.color-grid { display: grid; grid-template-columns: repeat(3, 1fr); gap: 12px; } .color-picker-group { display: flex; align-items: center; justify-content: space-between; padding: 8px 10px; background: var(--purecloud-script-bg-secondary); border: 1px solid var(--purecloud-script-border-color); border-radius: 4px; } .color-picker-group label { margin-bottom: 0; font-size: 0.85em; color: var(--purecloud-script-text-secondary); margin-right: 10px; font-weight: 600; white-space: nowrap; overflow: hidden; text-overflow: ellipsis; } .color-picker-group input[type="color"] { min-width: 40px; height: 25px; padding: 1px; border: 1px solid var(--purecloud-script-border-color); cursor: pointer; background: var(--purecloud-script-bg-primary); border-radius: 3px; flex-shrink: 0; } .color-picker-group input[type="color"]::-webkit-color-swatch-wrapper { padding: 0; } .color-picker-group input[type="color"]::-webkit-color-swatch { border: none; border-radius: 2px; } .progress-bar-compact { background-color: var(--mini-dashboard-progress-color); height: 100%; transition: width 0.5s ease-out; }
            .purecloud-script-custom-btn { background: var(--purecloud-script-bg-secondary) !important; border: 1px solid var(--purecloud-script-border-color) !important; padding: 5px 8px !important; margin: 0 2px !important; cursor: pointer; display: inline-flex; align-items: center; transition: all .2s; height: 30px; order: 3; color: var(--purecloud-script-text-secondary) !important; border-radius: 4px; font-size: 14px; text-decoration: none; position: relative; overflow: hidden; } .purecloud-script-custom-btn:hover { color: var(--purecloud-script-text-primary) !important; background-color: var(--purecloud-script-border-color) !important; box-shadow: 0 0 8px var(--purecloud-script-accent-color); transform: translateY(-1px); } .purecloud-script-custom-btn:active { background-color: var(--purecloud-script-accent-color) !important; color: var(--purecloud-script-accent-text) !important; box-shadow: inset 0 0 5px rgba(0,0,0,0.2); transform: translateY(0); } .purecloud-script-custom-btn svg { stroke: currentColor !important; width: 18px; height: 18px; margin-right: 0px !important; flex-shrink: 0; } .purecloud-script-custom-btn#purecloud-script-btn-toggleLock.lockdown-unlocked { cursor: default !important; opacity: 0.7; color: var(--purecloud-script-success-color) !important; background-color: var(--purecloud-script-bg-secondary) !important; box-shadow: none !important; transform: none !important; } .purecloud-script-custom-btn#purecloud-script-btn-toggleLock.lockdown-unlocked:hover { color: var(--purecloud-script-success-color) !important; background-color: var(--purecloud-script-bg-secondary) !important; }
            .purecloud-script-popup { position: fixed; background: var(--purecloud-script-bg-primary); border-radius: 8px; box-shadow: 0 8px 30px var(--purecloud-script-shadow-color); z-index: 10002; display: flex; flex-direction: column; border: 1px solid var(--purecloud-script-border-color); transition: opacity 0.2s, transform 0.2s, height 0.3s ease-in-out, width 0.3s ease-in-out, background-color 0.2s, border-radius 0.3s ease-in-out; margin: 0; overflow: hidden; } .purecloud-script-settings-panel { resize: none; width: 90%; max-width: 800px; min-height: 500px; height: 85vh; } .panel-header { cursor: move; background: var(--purecloud-script-bg-secondary); padding: 10px 15px; border-bottom: 1px solid var(--purecloud-script-border-color); display: flex; justify-content: space-between; align-items: center; border-radius: 8px 8px 0 0; } .panel-header h2, .panel-header h3 { margin: 0; color: var(--purecloud-script-text-primary); font-size: 16px; font-weight: 600; } .panel-header-buttons { display: flex; gap: 5px; } .panel-header-btn { background: none; border: none; font-size: 1.2em; cursor: pointer; color: var(--purecloud-script-text-secondary); padding: 0 5px; } .panel-header-btn:hover { color: var(--purecloud-script-text-primary); } .panel-content { padding: 15px; overflow-y: auto; flex-grow: 1; background: var(--purecloud-script-bg-primary); color: var(--purecloud-script-text-primary); } .panel-footer { padding: 10px 15px; background: var(--purecloud-script-bg-secondary); border-top: 1px solid var(--purecloud-script-border-color); text-align: right; border-radius: 0 0 8px 8px; } .purecloud-script-button.button-danger { background-color: var(--purecloud-script-error-color); color: #fff; }
            .purecloud-script-tabs { display: flex; background: var(--purecloud-script-bg-secondary); border-bottom: 1px solid var(--purecloud-script-border-color); padding-left: 10px; } .purecloud-script-tab-button { background-color: transparent; color: var(--purecloud-script-text-secondary); border: none; padding: 10px 15px; cursor: pointer; margin-right: 0px; transition: color 0.2s ease, border-bottom 0.2s ease; font-size: 0.95em; font-weight: 500; } .purecloud-script-tab-button.active { background-color: transparent; border-bottom: 2px solid var(--purecloud-script-accent-color); color: var(--purecloud-script-text-primary); font-weight: bold; } .purecloud-script-tab-button:hover:not(.active) { color: var(--purecloud-script-text-primary); } .purecloud-script-tab-content { display: none; padding-top: 15px; flex-direction: column; flex-grow: 1;} .purecloud-script-tab-content.active { display: flex; }
            .settings-group { margin-bottom: 20px; display: flex; flex-direction: column; } .settings-group label { color: var(--purecloud-script-text-secondary); font-size: 0.9em; margin-bottom: 5px; font-weight: 600; } .settings-group input[type="number"], .settings-group input[type="text"] { background: var(--purecloud-script-bg-primary); color: var(--purecloud-script-text-primary); border: 1px solid var(--purecloud-script-border-color); padding: 8px 10px; border-radius: 4px; transition: border-color 0.2s; width: 100%; box-sizing: border-box; } .settings-group input:focus { border-color: var(--purecloud-script-accent-color); outline: none; box-shadow: 0 0 0 2px rgba(76, 175, 80, 0.2); }
            ...
            .analytics-chart-container-main { padding: 15px 0 5px 0; } .chart-scroll-container { overflow-x: auto; width: 100%; padding-bottom: 5px; } .chart-bars-wrapper { display: flex; align-items: flex-end; height: 150px; min-width: 800px; padding-bottom: 20px; border-bottom: 1px solid var(--purecloud-script-border-color); } .chart-bar-wrapper { width: 4.1666%; margin-right: 1px; text-align: center; position: relative; font-size: 0.7em; height: 100%; flex-shrink: 0; } .chart-bar { background-color: var(--purecloud-script-accent-color); border-radius: 2px 2px 0 0; transition: height 0.3s; } .bar-count { position: absolute; top: -15px; left: 50%; transform: translateX(-50%); font-size: 0.8em; font-weight: bold; color: var(--purecloud-script-text-primary); } .bar-label { position: absolute; bottom: -15px; left: 0; width: 100%; color: var(--purecloud-script-text-secondary); } .panel-content h4 { color: var(--purecloud-script-text-primary); font-size: 1.1em; margin-top: 15px; margin-bottom: 10px; border-bottom: 1px solid var(--purecloud-script-border-color); padding-bottom: 5px; }
            div.interaction-group{position:relative!important;} #purecloud-script-mini-dashboard { padding: 0; background-color: var(--purecloud-script-bg-secondary) !important; box-shadow: 0 4px 10px var(--purecloud-script-shadow-color); border: 1px solid var(--purecloud-script-border-color); resize: none; } #purecloud-script-mini-dashboard.collapsed { width: auto; min-width: 70px; height: 35px; border-radius: 50px; cursor: move; overflow: hidden; padding: 0 12px; } #purecloud-script-mini-dashboard.expanded { min-width: 300px; max-width: 300px; border-radius: 8px; cursor: default; resize: none; bottom: auto; left: auto; padding: 0; } .mini-dashboard-header { background-color: transparent; padding: 0; display: flex; align-items: center; justify-content: center; height: 100%; cursor: move; border-bottom: none; border-radius: inherit; transition: background-color 0.2s, justify-content 0.2s; } #purecloud-script-mini-dashboard.collapsed .mini-dashboard-header { cursor: move; } #purecloud-script-mini-dashboard.expanded .mini-dashboard-header { background-color: var(--purecloud-script-bg-secondary); border-bottom: 1px solid var(--purecloud-script-border-color); cursor: move; height: 35px; padding: 5px 10px; justify-content: space-between; } .mini-dashboard-content { padding: 10px; background-color: var(--purecloud-script-bg-primary); border-top: none; border-radius: 0 0 8px 8px; flex-grow: 1; overflow-y: auto; display: none; } #purecloud-script-mini-dashboard.expanded .mini-dashboard-content { display: block; } .counter-display { display: flex; align-items: center; flex-grow: 1; margin-right: 0; overflow: hidden; justify-content: center; } #purecloud-script-mini-dashboard.expanded .counter-display { justify-content: flex-start; margin-right: 5px; } .counter-text { color: var(--purecloud-script-text-primary); font-weight: bold; font-size: 1.1em; white-space: nowrap; } #purecloud-script-mini-dashboard.expanded .counter-text { margin-right: 10px; } .progress-bar-container-compact { flex-grow: 1; height: 4px; background-color: var(--purecloud-script-border-color); border-radius: 2px; overflow: hidden; display: none; } #purecloud-script-mini-dashboard.expanded .progress-bar-container-compact { display: block; } .mini-dashboard-controls { display: flex; align-items: center; gap: 5px; } #purecloud-script-mini-dashboard.collapsed .mini-dashboard-controls { display: none; } #purecloud-script-mini-dashboard.expanded .mini-dashboard-controls { display: flex; } .compact-btn { background: var(--purecloud-script-bg-primary); border: 1px solid var(--purecloud-script-border-color); color: var(--purecloud-script-text-secondary); border-radius: 4px; cursor: pointer; height: 25px; width: 25px; display: flex; align-items: center; justify-content: center; font-size: 0.8em; padding: 0; } .compact-btn:hover { color: var(--purecloud-script-text-primary); background-color: var(--purecloud-script-border-color); }
            ...
            .switch { position: relative; display: inline-block; width: 44px; height: 24px; } .switch input { opacity: 0; width: 0; height: 0; } .slider { position: absolute; cursor: pointer; top: 0; left: 0; right: 0; bottom: 0; background-color: var(--purecloud-script-border-color); transition: .4s; } .slider:before { position: absolute; content: ""; height: 16px; width: 16px; left: 4px; bottom: 4px; background-color: white; transition: .4s; } input:checked + .slider { background-color: var(--purecloud-script-success-color); } input:focus + .slider { box-shadow: 0 0 1px var(--purecloud-script-success-color); } input:checked + .slider:before { transform: translateX(20px); } .slider.round { border-radius: 24px; } .slider.round:before { border-radius: 50%; }
        `;
    ...
    function initializeDailyCounters(forceReset = false) {
        ...
        if (lastSavedDate !== todayKey || forceReset) { localStorage.setItem('purecloudScript_lastActivityDate', todayKey); analyticsManager.clearAllData(); } else { analyticsManager.getData(); loadV4Counters(); } 
    }

    function sincronizarDadosComPlanilha() {
        console.log('[Sincronização V2.1] Iniciando envio de dados via DataSyncer...');
        if (typeof window.analyticsManager === 'undefined' || typeof window.v4_counters === 'undefined') { console.warn('[Sincronização] Analytics não está pronto.'); return; }

        let currentUserName = "Usuário Anônimo";
        let userEl = document.querySelector('div.name span.entry-value');
        if (userEl) { currentUserName = userEl.innerText; }
        ...
        const analyticsPayload = {
            ...
            inicio: stats.last, 
            ultima: stats.first, 
            meta: CONFIG.CONVERSATION_TARGET,
            transferidos: stats.transferClicks,
            tmaGeralSegundos: stats.tmaSeconds || 0,
            tmeAtivoSegundos: stats.tmeSeconds || 0
        };
        DataSyncer.send({ type: 'analytics', user: currentUserName, stats: analyticsPayload });

        const atendimentosPayload = analyticsManager.getData().conversations.map(conv => ({
            ...
            tme: (() => { const s = Math.floor(conv.activeDuration / 1000); return `${Math.floor(s/60).toString().padStart(2,'0')}:${(s%60).toString().padStart(2,'0')}` })(),
            tmeSegundos: Math.floor(conv.activeDuration / 1000), 
            recorrencia: conv.isRecurrence ? 'Sim' : 'Não',
            link: conv.interactionUrl || 'N/A'
        }));
        DataSyncer.send({ type: 'atendimento', user: currentUserName, atendimentos: atendimentosPayload });

        if (v4_counters.balao.length > 0) {
             DataSyncer.send({ type: 'encerrados', user: currentUserName, encerrados: v4_counters.balao });
        }
    }

    // ==========================================================
    // INICIALIZAÇÃO COM "PORTEIRO" (VERIFICAÇÃO DE PERMISSÃO)
    // ==========================================================
    
    const initialize = () => {
        // Tenta verificar se é uma Extensão do Chrome com acesso a storage
        if (typeof chrome !== 'undefined' && chrome.storage && chrome.storage.local) {
            // O "Porteiro": Verifica na memória da extensão se deve mostrar as ferramentas
            chrome.storage.local.get(['SHOW_AGENT_TOOLS'], function(result) {
                // Lógica: Se não existir configuração (undefined), assume TRUE (mostra). 
                const deveMostrar = result.SHOW_AGENT_TOOLS !== false; 
                iniciarScript(deveMostrar);
            });
        } else {
            // Fallback para Tampermonkey
            log("Ambiente não é extensão ou sem acesso a storage. Carregando modo padrão.");
            iniciarScript(true);
        }
    };

    // Função interna que carrega as coisas baseada na decisão do porteiro
    function iniciarScript(mostrarFerramentas) {
        // 1. Funcionalidades de Background (Sempre rodam, mesmo oculto)
        setInterval(() => DataSyncer.processQueue(), 60000); 
        setTimeout(() => DataSyncer.updateIndicator(), 3000); 
        
        setTimeout(sincronizarDadosComPlanilha, 45000); 
        const DEZ_MINUTOS = 10 * 60 * 1000;
        setInterval(sincronizarDadosComPlanilha, DEZ_MINUTOS);
        console.log(`[Sincronização] Configurada com suporte Offline (DataSyncer).`);

        if (!mostrarFerramentas) {
            log("🚫 Modo Admin Silencioso: Ferramentas de Agente ocultas. Apenas sincronização ativa.");
            return; 
        }

        // 2. Funcionalidades Visuais (Só rodam se o porteiro deixar)
        log("✅ Modo Agente Ativo: Carregando interface...");
        ...
        if (!targetNode) { targetNode = document.body; log("WARN: Container de lista não encontrado. Usando document.body como alvo."); } 
        ...
        if (CONFIG.MINI_DASHBOARD_VISIBLE) { 
            setTimeout(() => { UI.createMiniDashboard(); UI.createNotification("Script operacional.", 'info', 4000); }, 4000); 
        }
    }
